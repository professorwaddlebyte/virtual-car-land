...
export default function ShortlistPage() {
  ...
  function planRoute() {
    ...
    // Sort by map_x coordinate (left to right across the market)
    const sorted = [...shortlist]
      .map((v, i) => ({ ...v, originalIndex: i }))
      .sort((a, b) => {
        const ax = parseFloat(a.map_x || 50);
        const bx = parseFloat(b.map_x || 50);
        return ax - bx;
      });
  ...
  return (
    <>
      <Head>
        <title>My Shortlist — Virtual Car Land</title>
      </Head>
        ...
        <header className="bg-white shadow-sm sticky top-0 z-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              ...
              {shortlist.length > 0 && (
                <button onClick={clearShortlist} className="text-sm text-red-400 hover:text-red-600">
                  Clear All
                </button>
              )}
            </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      {shortlist.length} car{shortlist.length !== 1 ? 's' : ''} saved
                    </p>
                  </div>
                  <button
                    onClick={planRoute}
                  ...
                  <div key={v.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
                    <div className="flex items-start p-4 gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-lg font-bold mt-1" style={{color: '#0055A4'}}>
                        ...
                        <div className="flex items-center gap-2 mt-2 p-2 bg-gray-50 rounded-xl">
                          ...
                          <span className={`ml-auto px-2 py-1 rounded-full text-xs font-medium ${tierColors[v.score_tier] || tierColors.Unrated}`}>
                            {v.score_tier}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="flex border-t border-gray-100">
                      ...
                      <button
                        ...
                        className="flex-1 py-3 text-center text-sm font-medium text-red-400 hover:text-red-600 border-l border-gray-100"
                      >
                        Remove
                      </button>
                    </div>
