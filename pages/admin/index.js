...
export default function AdminDashboard() {
  ...
  async function approveListing(vehicleId) {
    const token = localStorage.getItem('token');
    const res = await fetch(`/api/admin/approve/${vehicleId}`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
  ...
  async function rejectListing(vehicleId) {
    ...
    const token = localStorage.getItem('token');
    const res = await fetch(`/api/admin/approve/${vehicleId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
    const d = await res.json();
    if (d.ok) setPendingVehicles(prev => prev.filter(v => v.id !== vehicleId));
    else alert('Failed: ' + d.error);
  }
  ...
  async function deleteDealer(id) {
    ...
    const token = localStorage.getItem('token');
    const res = await fetch(`/api/admin/dealers/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
    const d = await res.json();
  ...
  async function deleteShowroom(id) {
    ...
    const token = localStorage.getItem('token');
    const res = await fetch(`/api/admin/showrooms/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
    const d = await res.json();
  ...
  const filteredDealers = dealers.filter(d => !dealerSearch || d.business_name?.toLowerCase().includes(dealerSearch.toLowerCase()) || d.phone?.includes(dealerSearch));
  const filteredShowrooms = showrooms.filter(s => !showroomSearch || s.showroom_number?.toLowerCase().includes(showroomSearch.toLowerCase()) || s.dealer_name?.toLowerCase().includes(showroomSearch.toLowerCase()));

  if (loading) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center"><div className="text-4xl mb-4">⏳</div><p className="text-gray-500">Loading admin dashboard...</p></div>
    </div>
  );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl p-6">
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setEditingDealer(null)} className="flex-1 py-2.5 rounded-xl text-gray-600 font-semibold bg-gray-100">Cancel</button>
              <button onClick={() => saveDealer(editingDealer)} className="flex-1 py-2.5 rounded-xl text-white font-bold" style={{ background: '#1A9988' }}>Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingShowroom && (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl p-6">
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setEditingShowroom(null)} className="flex-1 py-2.5 rounded-xl text-gray-600 font-semibold bg-gray-100">Cancel</button>
              <button onClick={() => saveShowroom(editingShowroom)} className="flex-1 py-2.5 rounded-xl text-white font-bold" style={{ background: '#1A9988' }}>Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => { localStorage.clear(); router.push('/login'); }} className="text-white text-sm opacity-75 hover:opacity-100">Logout</button>
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="max-w-7xl mx-auto px-4 py-6 space-y-5 flex-1 w-full">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            <StatCard icon="⏳" label="Pending Approval" value={pendingVehicles.length} color={pendingVehicles.length > 0 ? '#d97706' : '#374151'} />
          ...
          {activeTab === 'pending' && (
            <div className="space-y-3">
              <div className="bg-white rounded-2xl p-5 shadow-sm">
                ...
                <p className="text-sm text-gray-500 mb-4">New listings with photos submitted by dealers — review and approve or reject</p>
                {pendingVehicles.length === 0 ? (
                  <div className="text-center py-10"><div className="text-4xl mb-3">✅</div><p className="text-gray-400 text-sm">No pending listings.</p></div>
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <p className="text-xs text-gray-400 mt-1">Dealer: {v.dealer_name} — Showroom {v.showroom_number}</p>
                            {v.description && <p className="text-xs text-gray-500 mt-1 italic">"{v.description}"</p>}
                          </div>
                          <span className="px-2 py-1 bg-orange-100 text-orange-700 text-xs font-semibold rounded-full flex-shrink-0">Pending</span>
                        </div>
                        {v.photos?.length > 1 && (
                          <div className="flex gap-1 mt-2">
                            ...
                            {v.photos.length > 5 && <span className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-xs text-gray-500">+{v.photos.length - 5}</span>}
                          </div>
                        )}
                        <div className="flex gap-2 mt-3">
                          <button onClick={() => approveListing(v.id)}
                            className="flex-1 py-2 rounded-xl text-white text-sm font-bold" style={{ background: '#1A9988' }}>
                            ✅ Approve & Publish
                          </button>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {[
                    { label: 'Export Vehicles CSV', href: '/api/admin/export?type=vehicles', icon: '📥' },
                    { label: 'Export Dealers CSV', href: '/api/admin/export?type=dealers', icon: '📥' },
                    { label: 'Export Inquiries CSV', href: '/api/admin/export?type=inquiries', icon: '📥' },
                  ].map((l, i) => (
          ...
          {activeTab === 'dealers' && (
            <div className="space-y-3">
              <div className="bg-white rounded-2xl p-4 shadow-sm flex gap-3">
                ...
                <button onClick={() => setEditingDealer({ business_name: '', phone: '', listing_integrity_score: 50, subscription_tier: 'Basic' })}
                  className="px-4 py-2.5 rounded-xl text-white text-sm font-bold" style={{ background: '#1A9988' }}>
                  + Add Dealer
                </button>
              </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex gap-3 mt-2">
                        ...
                        <span className="text-xs text-gray-400">{d.active_listings} active listings</span>
                        <span className="text-xs text-gray-400">Bot: {d.telegram_chat_id ? '✅' : '❌'}</span>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => setEditingDealer({ ...d })}
                        className="px-3 py-1.5 rounded-xl text-sm font-semibold bg-gray-100 text-gray-700">✏️ Edit</button>
                      <button onClick={() => deleteDealer(d.id)}
                        className="px-3 py-1.5 rounded-xl text-sm font-semibold bg-red-50 text-red-500">🗑️</button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-3">
              <div className="bg-white rounded-2xl p-4 shadow-sm flex gap-3">
                <input type="text" placeholder="🔍 Search showrooms..." value={showroomSearch}
                  ...
                  className="flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none" />
                <button onClick={() => setEditingShowroom({ showroom_number: '', section: '', location_hint: '', map_x: '', map_y: '' })}
                  className="px-4 py-2.5 rounded-xl text-white text-sm font-bold" style={{ background: '#1A9988' }}>
                  + Add Showroom
                </button>
              </div>
