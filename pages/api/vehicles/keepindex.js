...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (market_id) {
      conditions.push(`v.market_id = $${paramIndex++}`);
    ...
    if (model) {
      conditions.push(`LOWER(v.model) LIKE LOWER($${paramIndex++})`);
      params.push(`%${model}%`);
    }
    ...
    if (price_min) {
      conditions.push(`v.price_aed >= $${paramIndex++}`);
    ...
    if (transmission) {
      conditions.push(`LOWER(v.specs->>'transmission') = LOWER($${paramIndex++})`);
      params.push(transmission);
    }

    if (body) {
      conditions.push(`LOWER(v.specs->>'body') = LOWER($${paramIndex++})`);
      params.push(body);
    }
