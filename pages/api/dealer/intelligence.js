...
function aiFlag(v, avgPrice) {
  ...
  if (priceDiff !== null && priceDiff > 5) flags.push({ label: 'Above Market', color: 'red', action: `Priced ${priceDiff}% above market average. Consider reducing to enter competitive range.` });
...
// Market demand — views + sold combined, grouped by make/model/year
    ...
    if (priceResistance.length > 0) actions.push({ priority: 'medium', icon: '🚫', text: `${priceResistance.length} car${priceResistance.length > 1 ? 's have' : ' has'} high views but zero WhatsApp contact — buyers are looking but not inquiring. Review price or photos.`, vehicle_ids: priceResistance.map(v => v.id) });
